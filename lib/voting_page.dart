...
class Voting extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(home: votingpage());
  }
}
...
class _votingpage extends State<votingpage> with TickerProviderStateMixin {
  ...
  late StreamSubscription<QuerySnapshot> __trendingfeed,
      __livefeed,
      __endedfeed;
  ...
  bool endedloading = true;
  bool _lazyloading = false;

  getdata(
      {required StreamSubscription<QuerySnapshot> streamsubscription,
      required List<QueryDocumentSnapshot> streamer,
    ...
    if (!live) {
      __endedfeed.cancel();
      __endedfeed = FirebaseFirestore.instance
          .collection("polls")
          .where("endtime", isLessThanOrEqualTo: DateTime.now())
          .limit(widget.endcount)
          .snapshots()
          .listen((event) {
        setState(() {
      ...
      __livefeed.cancel();
      __livefeed = FirebaseFirestore.instance
          .collection("polls")
          .where("endtime", isGreaterThanOrEqualTo: DateTime.now())
          .limit(widget.livecount)
          .snapshots()
          .listen((event) {
        setState(() {
          print("docs length is ${event.docs.length}");
  ...
  void initState() {
    ...
    __trendingfeed = FirebaseFirestore.instance
        .collection("polls")
            ...
            body: TabBarView(
              children: [
                SingleChildScrollView(
                    child: showpost(
                        trending: true,
                        fetchcondition: !trendloading ? _trendingfeed : null,
                        error: error,
                        uid: uid)),
                SingleChildScrollView(
                    controller: live,
                    child: showpost(
                        fetchcondition: !liveloading ? _livefeed : null,
                        error: error,
                ...
                SingleChildScrollView(
                    ...
                    child: showpost(
                        ended: true,
                        fetchcondition: !endedloading ? _endedfeed : null,
                        error: error,
                        lazyloading: _lazyloading,
                        endcount: widget.endcount,
                        livecount: null,
                        uid: uid))
              ],
            ),
...
class _optionforquestion extends State<optionforquestion> {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
        onTap: () async {
          if (!widget.ended) {
            List dummyuservoterresponse = await onSameVotePress(
              widget.uservoteresponse,
              widget.documentid,
              widget.multiple,
              widget.singletime,
              widget.index,
              widget.optionslength,
              widget.votecountlist,
              widget.uservoted,
            );
          } else {
            Fluttertoast.showToast(
                msg: "This poll has been ended",
          ...
          children: [
              ...
              decoration: BoxDecoration(
                ...
                color: uservoted
                    ? Colors.blue.withOpacity(
                        (widget.votecountlist[widget.index] /
                            widget.totalvotecount))
                    : Colors.white,
              ),
              width: uservoted
                  ? size *
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: value ? Colors.blue : Colors.black54),
              ),
              child: Row(children: [
                value
                ...
                uservoted
                    ? Row(children: [
                        ...
                        Text(
                          "${double.parse(((widget.votecountlist[widget.index] / widget.totalvotecount) * 100).toStringAsFixed(1))}%",
                  ...
                  child: SingleChildScrollView(
                      ...
                      child: Column(children: [
                          ...
                          itemBuilder: (context, i) {
                            return Padding(
                                ...
                                child: TextFormField(
                                  maxLength: 30,
                                  validator: (v) =>
                                      _validateoption(v, textcontrollers),
                        ...
                        Row(children: [
                          ...
                          const Expanded(
                              child: Text(
                            "Make the post visible to only those who have the link.",
                          ))
                        ]),
...
Widget showpost(
    ...
    trending: false,
    ended: false,
    optionslengthlarge: false,
    required List<QueryDocumentSnapshot>? fetchcondition,
    DocumentSnapshot? individualfetch,
    required error,
    lazyloading: false,
    livecount: null,
    endcount: null,
    ...
    return Builder(builder: (context) {
      ...
      if (error == true) {
        return Column(children: [
          LottieBuilder.asset(
            "assets/lottiefiles/85978-error-dialog.json",
...
Widget pollpostdesign(
  ...
  String lefttime =
      "${endtime.toDate().difference(DateTime.now()).inDays} Days";
  bool isoptionslenthlarge = (options.length > 4);
  if (endtime.toDate().difference(DateTime.now()).inSeconds < 0) {
    ended = true;
  }
  if (endtime.toDate().difference(DateTime.now()).inDays == 0) {
    if (endtime.toDate().difference(DateTime.now()).inHours > 0) {
      ...
      lefttime = "${endtime.toDate().difference(DateTime.now()).inMinutes} min";
    }
  }
                      ...
                      children: [
                        const CircleAvatar(
                          ...
                          foregroundImage: NetworkImage(
                              "https://thumbs.dreamstime.com/b/girl-vector-icon-elements-mobile-concept-web-apps-thin-line-icons-website-design-development-app-premium-pack-glyph-flat-148592081.jpg"),
                        ),
                        const SizedBox(
                          width: 5,
                        ),
                        ...
                        const Spacer(),
                        mypolls
                            ? mypollspopupmenu(context, documentid)
                            : popupmenu(context, documentid)
                      ],
                    )),
                Padding(
                    padding: const EdgeInsets.fromLTRB(10, 1, 10, 15),
                    child: forlargequestions(context, question)),
                Column(
                    children: !isoptionslenthlarge
                        ? options
                            .asMap()
                            .entries
                            .map((items) => optionforquestion(
                                documentid,
                                items.value,
                                ismultiple,
                                items.key,
                                ...
                                ended))
                            .toList()
                        : [largeoptioncontainer(context, documentid)]),
                const SizedBox(
                  height: 3,
                ),
                Row(children: [
                  OutlinedButton(
                      ...
                      onPressed: () {
                        newOnLikePress(
                            documentid, isliked, userid, "polls", _isuservoted);
                      },
                      child: Column(children: [
                        Icon(
                          isliked
                  ...
                  IconButton(
                      onPressed: () async {
                        var linkresult =
                            await firebasedynamiclink("polls", documentid);
                        if (linkresult != false || linkresult != "out") {
                          Share.share(
                              "express your opnion to the poll $linkresult");
                        } else {
                          Fluttertoast.showToast(
                              msg: "unable to generate link",
                              backgroundColor: Colors.red,
                              timeInSecForIosWeb: 3);
                        }
                      },
        ...
        builder: (context) => Scaffold(
              ...
              body: SingleChildScrollView(
                child: showpost(
                    ...
                    optionslengthlarge: true,
                    fetchcondition: null,
                    individualfetch: !loading ? _feed : null,
                    error: error,
                    ended: !loading
                        ? !(_feed["endtime"]
                                .toDate()
...
String? _validateoption(String? value, List controllers) {
  ...
  controllers.forEach((element) {
    if (optionstext.contains(element.text)) {
      _optionstext.add(true);
    } else {
...
pushingtofirestore(question, List optionslist, time, multipleoption, publicview,
    singletime) async {
  List options = [];
  List votes = [];
  List response = [];
  ...
  final DocumentReference newpoll =
...
showloadingdilog(BuildContext context, question, optionslist, time,
    multipleoption, bool publicview, singletime) {
  var document_id;
  int count = 0;
  showDialog(
      context: context,
      ...
      builder: (_) => AlertDialog(
            ...
            content: FutureBuilder(
              future: pushingtofirestore(question, optionslist, time,
                  multipleoption, publicview, singletime),
              builder: (context, future) {
                if (future.hasData) {
                  if (future.data != false && future.data != "out") {
                    document_id = future.data;
                    return Text(
                        !publicview
                            ? "Successfully Posted"
                            : "Successfully Posted.Share your link by using share button",
            ...
            actions: [
              TextButton(
                  onPressed: () async {
                    ...
                    var linkresult =
                        await firebasedynamiclink("polls", document_id);
                    if (linkresult != false || linkresult != "out") {
                      Share.share(
                          "express your opnion to the poll $linkresult");
                    } else {
            ...
            child: Row(
                ...
                children: [
                  const SizedBox(
                    width: 180,
                    child: Text(
                        "This question contains too many options if you want to see click on view"),
                  ),
                  TextButton(
                      onPressed: () {
                        Navigator.of(context).push(MaterialPageRoute(
                            builder: (context) =>
                                individualpoll(documentid, true)));
                      },
                      child: const Text(
                        "View",
                        style: TextStyle(
